...
def GetOPPower(dbc,day,cp,sprice):
    ...
    #print BadPower
    return Contract,SPrice,MainPower,BadPower,Price
...
def WriteSignalToDB(dbc,dTable,dDate,dTime,dStrategy,dIndex,dPrice,dUnit,dWinPoint,dSprice,dContract):   
    ...
    if rcnt==0:
        SQL="Insert into %s(TDATE,TDATETIME,TIMEINDEX,Option_CurPrice,NowUnit,WinPoint,SignalTime,StrategyIndex,Option_Sprice,Option_Contract) Values('%s','%s',%d,%d,%d,%d,'%s','%s',%d,'%s')" %(dTable,dDate,dTime,dIndex,dPrice,dUnit,dWinPoint,dTime,dStrategy,dSprice,dContract)
        #print SQL        
        DBC.ExecDB(SQL)
...
def RunOPStrategy(day,Contract,SPrice,MainPower,BadPower,Price,CP):
    tradecnt=0
    unit=0
    Profit=0
    for i in range(len(MainPower)):
        ...
        if MainPower[i-1]>0.1 and BadPower[i-1]<-10 and unit==0 and tradecnt==0 and i>=5 and i<100 :         
            unit=1
            tradecnt+=1
            Profit=Price[i]
        ...
        if MainPower[i-1]<-0.1 and BadPower[i-1]>10 and unit==1:
            unit=0
            Profit=Profit-Price[i]-3
...
def Run(day):
    ...
    Contract,SPrice,MainPower,BadPower,Price=GetOPPower(opdb,day,"C",INP+50)
    TotalProfit+=RunOPStrategy(day,Contract,SPrice,MainPower,BadPower,Price,"C")

    Contract,SPrice,MainPower,BadPower,Price=GetOPPower(opdb,day,"C",INP)
    TotalProfit+=RunOPStrategy(day,Contract,INP,MainPower,BadPower,Price,"C")
    ...
    #遇到下跌容易爆噴，不考慮
    Contract,SPrice,MainPower,BadPower,Price=GetOPPower(opdb,day,"P",INP-50)
    TotalProfit+=RunOPStrategy(day,Contract,SPrice,MainPower,BadPower,Price,"P")

    #遇到下跌容易爆噴，不考慮
    Contract,SPrice,MainPower,BadPower,Price=GetOPPower(opdb,day,"P",INP)
    TotalProfit+=RunOPStrategy(day,Contract,SPrice,MainPower,BadPower,Price,"P")
    
    Contract,SPrice,MainPower,BadPower,Price=GetOPPower(opdb,day,"P",INP+50)
    TotalProfit+=RunOPStrategy(day,Contract,SPrice,MainPower,BadPower,Price,"P")    
    
    Contract,SPrice,MainPower,BadPower,Price=GetOPPower(opdb,day,"P",INP+100)
    TotalProfit+=RunOPStrategy(day,Contract,SPrice,MainPower,BadPower,Price,"P")
