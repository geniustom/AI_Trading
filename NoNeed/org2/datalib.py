...
class AutoTrade:
    def __init__(self,host,uid,pwd):
        ...
        self.dc=DB(self.connstr)
        self.Qy=Query(self.dc.dbconn)
        r, rcnt = self.Qy.QueryDB("SELECT TDATE FROM (SELECT DISTINCT TDATE FROM RealTimeFuture) as NEW ORDER BY TDATE")
        rr=r.GetRows(rcnt)   
        self.DateList=[]
    ...
    def DayTrade(self,day):
        ...
        self.indi=self.Qy.QueryDBtoIndicators(SQL)
        self.indi.GetBaseIndicator()
        print self.indi.get("大台指數")[1:10]
        return self.indi 
