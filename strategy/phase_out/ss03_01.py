...
def s1(self,PRICE,i,I): #1763
    ...
    tx=I.get("小台未純化主力作為")[i-1]
    mf=I.get("金期未純化主力作為")[i-1]
    me=I.get("電期未純化主力作為")[i-1]
    
    if tx<-1 and (mf+me)/2>tx : self.EnterShort(PRICE)
    if tx>1  and (mf+me)/2<tx : self.EnterLong(PRICE)
    
    self.CheckDailyExitAll(I.get("TIME")[i],PRICE)

############################################################################### 
import os
STittle=u"[ss03_01]金電加總與小台未純化主力企圖同步性"
