...
def filter1(self,I):  
    ...
    if self.RunToday==-1:
        ...
        if (f1>20 and f1<26) or (f2<8 and f3==0) :
            self.RunToday=0
...
def s1(self,PRICE,i,I): #小主小黑無濾網1575 小主大黑無濾網1971 (42,67)1497
    if filter1(self,I)==0:
        return
    if I.get("小台未純化主力企圖動能")[i-1]<0 and I.get("大台黑手")[i-1]<0 : self.EnterShort(PRICE)
    if I.get("小台未純化主力企圖動能")[i-1]>0 and I.get("大台黑手")[i-1]>0 : self.EnterLong(PRICE)
...
############################################################################### 
import os
STittle=u"[s05_2]小台主力企圖動能、大台黑手策略"
