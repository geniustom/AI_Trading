...
class NN:
    ...
    def test(self, patterns):
        ...
        for p in patterns:
            inputs = p[0]
            opt=self.runNN(inputs)
            #print 'Inputs:', p[0], '-->', self.runNN(inputs), '\tTarget', p[1]
            print 'Inputs:',opt , '\tTarget', p[1]
            result.append([opt,p[1]])
        return result
...
if __name__ == "__main__":
    ...
    train_pat=[]
    test_pat=[]
    for i in range(TrainTag.shape[0]):
        S=TrainData[i,:].tolist()
        T=TrainTag.T[i]
        if i<training_count:
            train_pat.append([S,[T]])
        else:
            test_pat.append([S,[T]])
